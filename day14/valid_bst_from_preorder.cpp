...
// making a hypothetical bst using preorder

// traversing through preorder vector and checking if the index stays less than size return false

void makeBST(vector<int> &A, int &index, int low, int high)
{

 if (index >= A.size())
  return; // preorder vector exhausted, means whole tree can be created

 if (A[index] <= low || A[index] >= high)
  return; // it means element cannot be pushed to either left or right
...
int solve(vector<int> &A)
{
 ...
 if (index < A.size())
  return false; // makeBST function returned in between, it was not possible to make a BST
